...
**Раскатка до 15 минут.** После каждого сохранения ресурса коды ответа скачут
между `000`, `502` и `200`: узлы получают новую конфигурацию не одновременно.
Повторные правки только перезапускают отсчёт — сохранили и ждите.

**Отрицательный DNS-кэш.** Если CDN какое-то время ходил на несуществующее имя,
часть узлов помнит это и продолжает отдавать `502` ещё минут десять после того,
как вы всё починили.

**Смена origin при переустановке.** Каждый запуск установщика генерирует новый
поддомен origin, и ресурс приходится править. Чтобы этого не было, запускайте с
флагом:

```bash
sudo python3 node-installer-cdn.py --origin-domain a7f3k2.example.com
```

**Клиенту нужны параметры транспорта.** И подписка, и ссылка из карточки несут
`extra` с настройками XHTTP — аплинк GET'ами и обфускация паддинга. Ссылка,
собранная вручную без `extra`, не заработает: клиент уйдёт на POST, а CDN
ответит `405`. При смене настроек в панели клиенту нужно **обновить подписку**,
иногда — удалить профиль и добавить заново.
