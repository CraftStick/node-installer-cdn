...
Здесь всё проще, чем у Yandex: домен источника не нужен, сертификат выпускать
негде и незачем, а настройки по умолчанию уже такие, как надо.

### Шаг 1. Создание ресурса

**Панель Timeweb → CDN → Добавить CDN-ресурс**

**1. Источник контента** — вкладка **«IP-адрес»** (не «Домен» и не «S3-бакет»):
...
> [!IMPORTANT]
> Порт указывайте явно. Без него Timeweb подставляет 443, пойдёт туда по HTTPS
> и упрётся в самоподписанный сертификат — ресурс будет отвечать `502`.

Тумблер HTTPS при создании включён, его нужно снять. Настоящий сертификат на
...
**3. Информация** — название любое, Timeweb предложит своё. Нажмите
**«Заказать»**.

### Шаг 2. Проверка настроек

Вкладка **«Управление»**. Всё, что ломает туннель, Timeweb по умолчанию держит
выключенным — достаточно пройтись и убедиться:
...
`xxxxxxxxxx.cdn.twcstorage.ru`. Его и вводите в установщик, а на вопрос про
свой домен нажимайте Enter: сертификат на техническом домене уже валидный,
клиентам его можно отдавать как есть.
...
> раздачу и на источник даже не ходит. Дождитесь статуса **«Активен»** —
> обычно 10–20 минут.

Проверять раскаткой по `curl` можно так:

```bash
curl -sSI https://xxxxxxxxxx.cdn.twcstorage.ru/health | grep -iE 'HTTP/|x-reason-code'
```

`403` с `x-reason-code: 7` — ещё раскатывается. `200` — ресурс в раздаче,
заглушка пришла через CDN.

### Шаг 4. Свой домен (необязательно)

**Управление → Источник и домены раздачи → Добавить домен** (до двух штук).
Затем CNAME у своего DNS-провайдера на технический домен, и на вкладке
**«SSL-сертификаты» → «Выпустить Let's Encrypt»** для этого домена. Без
сертификата клиент упрётся в ошибку TLS.

Вкладка «SSL-сертификаты» нужна **только** для своего домена. Надпись «Нет
подходящих сертификатов» при работе на техническом домене — нормально,
