...
Обе записи ведут на сам сервер. Домен панели нельзя направлять на CDN: Let's
Encrypt проверяет его прямо здесь, по webroot. Технический домен CDN выдаёт
провайдер, в свой DNS его заводить не нужно — установщик спросит его в конце и
сам пропишет в панель.
...
nginx перед панелью и CDN · сертификаты Let's Encrypt через certbot ·
профиль выбранного CDN · файрвол · BBR, TCP Fast Open и сетевые буферы ·
страницу-обманку на корне домена · подписки и клиентские ссылки ·
продолжение с места остановки, если прошлый запуск оборвался.
