...
## Почему только Yandex Cloud

Провайдер один, и выбирать не из чего — это не лень, а результат проверки.

**Timeweb убран.** Его CDN пропускает на источник только запросы, у которых
последний сегмент пути похож на файл, то есть имеет расширение. А XHTTP шлёт
`/<путь>/<сессия>/<номер>` — расширения там нет и взяться ему неоткуда: эти URL
собирает клиент. Каждый запрос туннеля возвращается с `403`, не доходя до
сервера. Замер с одного адреса:

| Запрос | Ответ |
|---|---|
| `/index.html` | `200` — прошло до источника |
| `/a/b/t.m3u8` | `404` от nginx — прошло |
| `/a/b/t.m3u8/abc123/0` | **`403` от CDN** — расширение в середине не считается |
| `/health` | **`403` от CDN** |

Обойти это со стороны клиента нельзя, поэтому провайдер и убран, чтобы не
обещать работу там, где её нет.

**VK Cloud и Beeline (CDNvideo) убраны раньше** по той же причине — не удалось
получить рабочий туннель.

Yandex Cloud ходит к источнику по HTTPS, то есть участок CDN → сервер шифруется,
и отдаёт 100+ Мбит/с. Технический домен он выдаёт свой
(`*.topology.gslb.yccdn.ru`), но отдавать его клиентам нельзя: вашего
сертификата на нём нет. Нужен свой домен с CNAME на технический — скрипт
придумает его сам, если не задать флагом.
