...
страницу-обманку на корне домена · подписки и клиентские ссылки · транзит через
РФ · автоматический откат, если что-то упало на середине.
...
| Вариант | Что даёт |
|---|---|
| **gRPC Reality** (TCP 2053) | рекомендуется: прямой вход, не зависящий от CDN |
| **Только CDN** | минимум открытых портов наружу |
| **+ Hysteria2** (UDP 8443) | ⚠️ по умолчанию выключен |

Про Hysteria2 честно: `hysteria2` — не протокол Xray, это отдельный проект.
Нода запускает стоковый xray-core, панель проверяет конфиг перед сохранением и
профиль с таким входом отвергает целиком. Флаг `--hy2` оставлен на случай сборки
с поддержкой, но по умолчанию канал выключен, а если панель откажет — установщик
пересоберёт профиль без него и скажет об этом.

Порты открываются ровно под выбранные каналы: отказались от gRPC — 2053 в ufw не
появится.
...
# инбаунды и транзит
--hy2             добавить Hysteria2 (по умолчанию выключен, см. ниже)
--no-hy2          не добавлять Hysteria2 (поведение по умолчанию)
