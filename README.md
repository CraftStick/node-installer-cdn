...
Две A-записи на IP сервера, **без проксирования** (в Cloudflare — серое
облачко):
...
HTTP, и скорость иногда проседает.
...
**Панель Timeweb → CDN → Создать ресурс**

| Поле | Значение |
|---|---|
| Источник | вкладка «IP-адрес», IP вашего сервера, порт 80 |
| HTTPS к источнику | не включать |
| Кеш CDN и браузера | выключить |
| Всегда онлайн | выключить |
| Query-параметры | не игнорировать |
| HTTP/3 | выключить |
| Сжатие Gzip/Brotli | выключить |

Технический домен `xxxxx.cdn.twcstorage.ru` создаётся автоматически, сертификат
на нём уже валидный — его можно отдать клиентам как есть и в установщике
оставить поле своего домена пустым.

Если хотите свой домен, заведите CNAME на технический и выпустите для своего
домена сертификат в панели Timeweb — иначе клиент упрётся в ошибку TLS.
...
(открыты SSH, 80, 443 и порты включённых входов), включает BBR, добавляет swap,
...
`2222` и запасные входы не закрываются.
...
Основной вход — **XHTTP packet-up** через CDN, он ставится всегда. Путь туннеля
случайный, вида `/content/media/k3f9xa2b`. К нему можно добавить запасной вход
**gRPC Reality** — прямой, в обход CDN, на TCP `2053`.
...
| Соединение есть, трафика нет | клиенту нужны параметры транспорта. Они уже внутри подписки и `vless://`-ссылки из карточки; ссылка, собранная вручную без них, не заработает. После переустановки обновите подписку в клиенте, а если не помогло — удалите профиль и добавьте заново |
