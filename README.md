...
Порт открывается ровно под выбор: отказались от gRPC — 2053 в ufw не появится.

Hysteria2 в установщике **нет**, и это осознанно: `hysteria2` — не протокол
Xray, а отдельный проект со своим сервером. Нода запускает стоковый xray-core,
панель проверяет конфиг перед сохранением и профиль с таким входом отвергает
целиком. Поднять Hysteria2 можно только отдельным сервисом мимо панели — это
другая задача, а не флаг.
...
Ядро — Xray (VLESS, XHTTP, gRPC Reality). Работа с серверами — по SSH под root.
