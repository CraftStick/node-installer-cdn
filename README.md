...
A     panel.example.com   →  <IP сервера>    панель и её сертификат
```

Нужны три имени: источник для CDN, панель и домен для клиентов (на него
выпускается сертификат в Yandex Cloud). Мастер спросит, сгенерировать их или
ввести свои. При переустановке выбирайте «ввести свои» и вписывайте прежние
имена — тогда ресурс CDN и сертификат переделывать не придётся.

Имя источника случайное не для красоты: предсказуемое `origin.example.com`
первым попадает в словари, которыми ищут настоящий IP сервера за CDN. Панель
живёт на `panel.example.com` — её адрес не прячут, там всё равно вход по
паролю.

Без вопросов имена задаются флагами `--origin-domain`, `--panel-domain` и
`--client-domain`. Свой домен панели
пригодится, если упрётесь в недельный лимит Let's Encrypt после нескольких
переустановок подряд: `--panel-domain panel2.example.com` — это другое имя и
свой счётчик.
...
В форме участвуют **два разных домена**, и поля называются похоже. Три поля
подряд — про ваш сервер, и в них одно и то же имя. Четвёртое, ниже, — про
клиентов.

| Поле | Значение | Чей домен |
|---|---|---|
| Запрос контента | из одного источника | |
| Тип источника | сервер | |
| **Доменное имя источника** | `a7f3k2.example.com` | ваш сервер |
| Протокол к источникам | HTTPS | |
| Задать SNI вручную | включить | |
| **Имя SNI-хоста** | `a7f3k2.example.com` | ваш сервер |
| Заголовок Host | своё значение | |
| **Значение заголовка** | `a7f3k2.example.com` | ваш сервер |
...
Если перепутать источник и доменное имя местами, CDN пойдёт сам к себе.
...
- **HTTP-заголовки и методы** — разрешённые методы: **GET, HEAD, OPTIONS**,
  отметить все три
- **Дополнительно** — выгрузка логов и экранирование источников не нужны

POST в списке методов у Yandex нет вовсе. Это не мешает: установщик настраивает
туннель на аплинк GET-запросами.

> [!NOTE]
> Группа источников нужна только для нескольких серверов — для одного она лишний
> шаг. Если всё же создаёте ресурс через «Копирование конфигурации» с другого,
> проверьте источник: подтянется чужой, и CDN пойдёт не к вам. Смена SNI и
> заголовка Host это не лечит, ответом будет `502`.
...
```
$ORIGIN cdn.example.com                        ← ваш домен, он уже известен
CNAME   c5d6df029464d4c0.topology.gslb.yccdn.ru  ← нужно это
```

Копируйте вторую: её же установщик спросит в конце. Заведите:
...
| **4** | удалить всё, что ставил скрипт | [полное удаление](#полное-удаление) |
...
Запустите скрипт той же командой, что и при установке, и выберите в меню
пункт **4 — «Удалить всё, что ставил скрипт»**. Пункт появляется, только если
на сервере есть следы установки: каталоги проекта, свои контейнеры или свои
конфиги nginx. Или сразу флагом:

```bash
sudo python3 node-installer-cdn.py --uninstall
```

Снимает всё, что установщик клал на сервер: контейнеры, тома, образы и сети,
каталоги `/opt/remnawave` и `/opt/remnanode`, свои конфиги nginx и Caddy,
сертификат источника и страницу-заглушку, тюнинг sysctl и лимиты, swap-файл
вместе с записью в `fstab`, сохранённый прогресс, правила ufw на порт ноды,
сертификаты Let's Encrypt, которые выпускал сам, и пакеты — docker, nginx,
certbot, caddy.

Скрипт печатает список и спрашивает подтверждение; `--wipe` отвечает за вас.

Что он **не** тронет: ufw с его политикой и правилами для SSH, 80 и 443, чужие
сертификаты и остальные пакеты системы. Если на сервере есть чужие контейнеры,
docker не удаляется — сносятся только свои. То же с nginx: есть чужие сайты —
пакет остаётся, убираются только свои конфиги.
...
--uninstall       снести всё, что поставил установщик, и выйти

--origin-domain   домен источника для CDN (иначе случайный поддомен)
--panel-domain    домен панели (по умолчанию panel.<домен>)
...
| Клиент не подключается, пинг `n/a` | домен для клиентов не резолвится — не заведён CNAME на технический домен CDN. Проверить: `dig +short cdn.example.com @1.1.1.1` |
...
| После переустановки поменялись домены | в мастере выберите «Ввести свои» и впишите прежние имена (или передайте `--origin-domain` и `--client-domain`), тогда ресурс CDN и сертификат править не придётся |
