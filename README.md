...
- аккаунт у CDN-провайдера: **Yandex Cloud** или **Timeweb** (у обоих есть
  бесплатный тариф)
...
```
A     a7f3k2.example.com  →  <IP сервера>    источник для CDN (только Yandex)
A     example.com         →  <IP сервера>    панель и её сертификат
```

Первая нужна только для Yandex Cloud: он ходит к источнику по имени. У Timeweb
источник задаётся IP-адресом, имя нигде не участвует, и скрипт эту запись не
просит.

Имя поддомена для источника скрипт придумывает сам и показывает на шаге DNS —
заведите именно его. Имя случайное не для красоты: предсказуемое
`origin.example.com` первым попадает в словари, которыми ищут настоящий IP
сервера за CDN. Своё имя можно задать флагом `--origin-domain`.
...
Подробный разбор с картой полей — ниже: [Yandex Cloud](#yandex-cloud-cdn) и
[Timeweb](#timeweb-cdn).
...
| Провайдер | Связь с источником | Скорость | Регистрация |
|:-:|---|---|:-:|
| **Yandex Cloud** | HTTPS на домен источника | 100+ Мбит/с | открытая |
| **Timeweb** | HTTP на IP сервера, порт 80 | 50–80 Мбит/с | открытая |

**Yandex Cloud** — если важна скорость и шифрование на участке CDN → сервер.
Настройка длиннее: нужен сертификат в Certificate Manager и отдельная группа
источников.

**Timeweb** — если нужно быстро: ресурс создаётся в пару кликов, технический
домен сразу с валидным сертификатом. Взамен — трафик до вашего сервера идёт по
HTTP, и скорость иногда проседает. Сертификат Let's Encrypt для источника при
этом не выпускается: CDN к нему по HTTPS не ходит, а выпуск тратит недельный
лимит и публикует имя источника в CT-логах.

Технический домен провайдер выдаёт сам (`*.topology.gslb.yccdn.ru` у Yandex,
`*.cdn.twcstorage.ru` у Timeweb). У Timeweb его можно отдать клиентам как есть.
У Yandex — нельзя: вашего сертификата на нём нет, нужен свой домен с CNAME на
технический.
...
`_acme-challenge.cdn.example.com → <значение>.cm.yandexcloud.net`. Заведите её у
своего DNS-провайдера ровно в том виде, в каком она показана в консоли, и
дождитесь статуса **Issued** — это от 5 до 30 минут.
...
```
CNAME  cdn.example.com  →  xxxxxxxx.topology.gslb.yccdn.ru   (без проксирования)
```
...
Домен источника здесь не нужен — CDN ходит к серверу по IP. Настройки по
умолчанию уже такие, как надо. Основные грабли не в настройках, а в порядке
действий и в ожидании.

### Шаг 1. Создание ресурса

**Панель Timeweb → CDN → Добавить CDN-ресурс**

**1. Источник контента** — вкладка **«IP-адрес»** (не «Домен», не «S3-бакет»):

| Поле | Значение |
|---|---|
| IP-адрес | `<IP сервера>:80` |
| Использовать HTTPS соединение для источника | **снять галку** |

> [!IMPORTANT]
> Порт указывайте явно. Без него Timeweb берёт 443, идёт туда по HTTPS и
> упирается в самоподписанный сертификат — ресурс отдаёт `502`.

Галка HTTPS при создании стоит, её нужно снять. Настоящий сертификат на
источник не выпускается: по HTTP идёт только участок CDN → ваш сервер, клиент
до CDN идёт по HTTPS.

В источник вписывается **ваш сервер**. Домен, который вы собираетесь отдать
клиентам, источником быть не может: он ведёт на сам CDN, и получится петля.
...
Вкладка **«Управление»**. Всё, что ломает туннель, Timeweb по умолчанию держит
выключенным — достаточно убедиться:
...
| Контент и подключение | Ускорение загрузки больших файлов | выкл |
| Безопасность | Secure token | выкл |
| HTTP-заголовки | — | ничего не добавлять |

Кеш ломает туннель: он передаёт не файлы, а поток запросов, и закешированный
ответ уйдёт не тому клиенту.

### Шаг 3. Дождаться статуса «Доступен»

> [!IMPORTANT]
> Пока статус ресурса — **«Применяются настройки»**, CDN отдаёт `403` на **все**
> пути, включая `/`. Это не ошибка настройки: узел ещё не принял ресурс в
> раздачу и на источник даже не ходит. Обычно 10–20 минут.

```bash
curl -sSI https://xxxxxxxxxx.cdn.twcstorage.ru/ | head -3
```
...
Дашборд ресурса → блок **«Ресурсы»** → строка **«Домен»**, вид
`xxxxxxxxxx.cdn.twcstorage.ru`. Копируйте кнопкой, не набирайте руками: там
легко спутать `0` и `O`, `l` и `1`.

Его и вводите в установщик, а на вопрос про свой домен нажимайте Enter:
сертификат на техническом домене уже валидный, клиентам его можно отдавать как
есть.

Вкладка «SSL-сертификаты» при этом пишет «Нет подходящих сертификатов» — это
нормально, она только про свои домены.

### Шаг 5. Свой домен (необязательно)

Порядок жёсткий, иначе сертификат не выпустится.

**1.** Управление → Источник и домены раздачи → **«+ Добавить домен»** (до двух
своих поддоменов), вписать и сохранить.

**2.** Завести CNAME у своего DNS-провайдера:

| Поле | Значение |
|---|---|
| Name | `<поддомен>` — только имя, без зоны |
| Target | `xxxxxxxxxx.cdn.twcstorage.ru` |
| Proxy | без проксирования (серое облачко) |

Слева — ваше имя, справа — чужое. Cloudflare показывает итог строкой
«`ваш.домен` is an alias of `технический`» — если написано наоборот, поля
перепутаны.

**3.** Проверить, что запись действительно попала в зону. Она может
отображаться в панели DNS и при этом не отдаваться:

```bash
dig +short NS example.com                       # узнать авторитетный сервер
dig +short cdn.example.com @martha.ns.cloudflare.com
```

Авторитетный сервер должен вернуть технический домен CDN, а обычный резолвер
(`@1.1.1.1`) — ещё и IP-адреса.

**4.** Только теперь **«SSL-сертификаты» → «Выпустить Let's Encrypt»**. Пока
домен не резолвится, Timeweb отвечает «Не у всех доменов правильно настроены
DNS-записи». Выпуск занимает несколько минут.

> [!WARNING]
> Пока добавленный свой домен не доведён до конца, ресурс может отдавать `403`
> и на техническом домене тоже. Либо доделайте домен, либо уберите его
> крестиком из списка доменов раздачи.

### Если `403` держится после статуса «Доступен»

Узел кеширует отказы, и сброс кеша (иконка метёлки на странице ресурса)
расходится по узлам не сразу. Проверяйте адресом, который ещё не запрашивали —
на нём кэша заведомо нет:

```bash
curl -sSI https://xxxxxxxxxx.cdn.twcstorage.ru/health | grep -iE 'HTTP/|x-reason-code|x-cdn-edge-cache'
```

`x-cdn-edge-cache: HIT` на свежем адресе означает, что узел отвечает, не ходя на
источник, — дело не в вашем сервере.
...
# повторная установка
--wipe            удалить прошлую установку без вопросов (для автозапуска)
...
--origin-le       выпустить его и там, где он не нужен (Timeweb)
